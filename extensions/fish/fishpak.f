...
C
C
      REAL*8       F(IDIMF,*) ,BDTS(1)    ,BDTF(1)    ,BDPS(1)    ,
     1                BDPF(1)    ,W(1)
      NBR = NBDCND+1
!!!      PI = 3.14159265358979
      pi   = 3.14159267
      ...
      IERROR = 9
      IF(IDIMF-M .LT. 0) GOTO 119
      IF(IDIMF-M .EQ. 0) GOTO 119
      IF(IDIMF-M .GT. 0) GOTO 101
  101 IERROR = 1
      IF(TS .LT. 0) GOTO 119
      IF(TS .EQ. 0) GOTO 102
      IF(TS .GE. 0) GOTO 102
  102 CONTINUE
      IF(PI-TF+EPS .LT. 0) GOTO 119
      IF(PI-TF+EPS .EQ. 0) GOTO 119
      IF(PI-TF+EPS .GT. 0) GOTO 103
  103 IERROR = 2
      IF(TF-TS .LT. 0) GOTO 119
      IF(TF-TS .EQ. 0) GOTO 119
      IF(TF-TS .GT. 0) GOTO 104
  104 IERROR = 3
      IF(MBDCND .LT. 0) GOTO 119
      IF(MBDCND .EQ. 0) GOTO 119
      IF(MBDCND .GT. 0) GOTO 105
  105 CONTINUE
      IF(MBDCND-9 .LT. 0) GOTO 106
      IF(MBDCND-9 .EQ. 0) GOTO 106
      IF(MBDCND-9 .GT. 0) GOTO 119
  106 IERROR = 4
      IF(PF-PS .LT. 0) GOTO 119
      IF(PF-PS .EQ. 0) GOTO 119
      IF(PF-PS .GT. 0) GOTO 107
  107 IERROR = 5
CCCCC IF (NCHECK(N)-2) 108,119,108
  108 IERROR = 6
      IF(NBDCND .LT. 0) GOTO 119
      IF(NBDCND .EQ. 0) GOTO 109
      IF(NBDCND .GT. 0) GOTO 109
  109 CONTINUE
      IF(NBDCND-4 .LT. 0) GOTO 110
      IF(NBDCND-4 .EQ. 0) GOTO 110
      IF(NBDCND-4 .GT. 0) GOTO 119
  110 IERROR = 7
      IF(MBDCND-4 .LT. 0) GOTO 112
      IF(MBDCND-4 .EQ. 0) GOTO 112
      IF(MBDCND-4 .GT. 0) GOTO 111
  111 GO TO (112,119,119,112,119),NBR
  112 IERROR = 8
      IF(ELMBDA .LT. 0) GOTO 113
      IF(ELMBDA .EQ. 0) GOTO 113
      IF(ELMBDA .GT. 0) GOTO 118
  113 IERROR = 10
      IF(TS .LT. 0) GOTO 114
      IF(TS .EQ. 0) GOTO 114
      IF(TS .GE. 0) GOTO 115
  ...
  115 CONTINUE
      IF(PI-TF-EPS .LT. 0) GOTO 116
      IF(PI-TF-EPS .EQ. 0) GOTO 116
      IF(PI-TF-EPS .GE. 0) GOTO 117
...
C
      ...
      DO 102 I=1,MP1
         ...
         THETA = FIM1*DTH+TS
         SINT(I) = SIN(THETA)
         IF(SINT(I) .LT. 0) GOTO 101
         IF(SINT(I) .EQ. 0) GOTO 102
         IF(SINT(I) .GT. 0) GOTO 101
...
C
  112 ITSP = ITS+1
      ...
      MUNK = ITF-ITS+1
      IF(ISP .LT. 0) GOTO 116
      IF(ISP .EQ. 0) GOTO 116
      IF(ISP .GT. 0) GOTO 113
  ...
  115 CONTINUE
      SS(M+1) = 1.
  116 CONTINUE
      IF(INP .LT. 0) GOTO 120
      IF(INP .EQ. 0) GOTO 120
      IF(INP .GT. 0) GOTO 117
  117 SN(1) = 1.
      D(ITF) = AM(ITF)/BM(ITF)
      IID = ITF-2
      DO 118 II=1,IID
...
C
  ...
  131 CONTINUE
      IF(ELMBDA .LT. 0) GOTO 138
      IF(ELMBDA .EQ. 0) GOTO 132
      IF(ELMBDA .GT. 0) GOTO 132
  132 ISING = 1
      SUM = WTS*WPS+WTS*WPF+WTF*WPS+WTF*WPF
      IF(INP .LT. 0) GOTO 134
      IF(INP .EQ. 0) GOTO 134
      IF(INP .GT. 0) GOTO 133
  133 SUM = SUM+WP
  134 CONTINUE
      IF(ISP .LT. 0) GOTO 136
      IF(ISP .EQ. 0) GOTO 136
      IF(ISP .GT. 0) GOTO 135
  135 SUM = SUM+WP
  136 SUM1 = 0.
      DO 137 I=ITSP,ITFM
         SUM1 = SUM1+SINT(I)
  ...
  139 CONTINUE
      IF(NBDCND-3 .LT. 0) GOTO 146
      IF(NBDCND-3 .EQ. 0) GOTO 140
      IF(NBDCND-3 .GT. 0) GOTO 146
  ...
  147 CONTINUE
      IF(NBDCND-3 .LT. 0) GOTO 154
      IF(NBDCND-3 .EQ. 0) GOTO 148
      IF(NBDCND-3 .GT. 0) GOTO 154
  ...
  164 CONTINUE
      PERTRB = 0.
      IF(ISING .LT. 0) GOTO 165
      IF(ISING .EQ. 0) GOTO 176
      IF(ISING .GT. 0) GOTO 165
  165 SUM = WTS*WPS*F(ITS,JPS)+WTS*WPF*F(ITS,JPF)+WTF*WPS*F(ITF,JPS)+
     1      WTF*WPF*F(ITF,JPF)
      IF(INP .LT. 0) GOTO 167
      IF(INP .EQ. 0) GOTO 167
      IF(INP .GT. 0) GOTO 166
  166 SUM = SUM+WP*F(1,JPS)
  167 CONTINUE
      IF(ISP .LT. 0) GOTO 169
      IF(ISP .EQ. 0) GOTO 169
      IF(ISP .GT. 0) GOTO 168
...
C
  ...
  178 CONTINUE
      NBDC = NBDCND
      IF(NBDCND-4 .LT. 0) GOTO 182
      IF(NBDCND-4 .EQ. 0) GOTO 179
      IF(NBDCND-4 .GT. 0) GOTO 182
  179 JFN = NUNK/2
      DO 181 J=1,JFN
         JFRD = J+JPS-1
         JBRD = JPF-J+1
  ...
  182 CALL POIS (INTL,NBDC,NUNK,MBDCND,MUNK,AM(ITS),BM(ITS),CM(ITS),
     1           IDIMF,F(ITS,JPS),IERROR,D)
      IF(NBDCND-4 .LT. 0) GOTO 186
      IF(NBDCND-4 .EQ. 0) GOTO 183
      IF(NBDCND-4 .GT. 0) GOTO 186
  183 DO 185 J=1,JFN
         JFRD = J+JPS-1
         JBRD = JPF-J+1
         DO 184 I=ITS,ITF
  ...
  185 CONTINUE
  186 CONTINUE
      IF(ISING .LT. 0) GOTO 194
      IF(ISING .EQ. 0) GOTO 194
      IF(ISING .GT. 0) GOTO 187
  187 CONTINUE
      IF(INP .LT. 0) GOTO 191
      IF(INP .EQ. 0) GOTO 191
      IF(INP .GT. 0) GOTO 188
  188 CONTINUE
      IF(ISP .LT. 0) GOTO 189
      IF(ISP .EQ. 0) GOTO 189
      IF(ISP .GT. 0) GOTO 194
  189 DO 190 J=1,NP1
         F(1,J) = 0.
  190 CONTINUE
      GO TO 217
  191 CONTINUE
      IF(ISP .LT. 0) GOTO 194
      IF(ISP .EQ. 0) GOTO 194
      IF(ISP .GT. 0) GOTO 192
  192 DO 193 J=1,NP1
         F(M+1,J) = 0.
  193 CONTINUE
      GO TO 217
  194 CONTINUE
      IF(INP .LT. 0) GOTO 201
      IF(INP .EQ. 0) GOTO 201
      IF(INP .GT. 0) GOTO 195
  ...
  196 CONTINUE
      ...
      IF(ISP .LT. 0) GOTO 197
      IF(ISP .EQ. 0) GOTO 197
      IF(ISP .GT. 0) GOTO 202
  ...
  200 CONTINUE
      GO TO 217
  201 CONTINUE
      IF(ISP .LT. 0) GOTO 217
      IF(ISP .EQ. 0) GOTO 217
      IF(ISP .GT. 0) GOTO 202
  ...
  203 CONTINUE
      ...
      IF(INP .LT. 0) GOTO 204
      IF(INP .EQ. 0) GOTO 204
      IF(INP .GT. 0) GOTO 208
  ...
  208 RTN = F(1,1)-DFN
      RTS = F(M+1,1)-DFS
      IF(ISING .LT. 0) GOTO 210
      IF(ISING .EQ. 0) GOTO 210
      IF(ISING .GT. 0) GOTO 209
  209 CSP = 0.
      CNP = RTN/DNN
      GO TO 213
  210 CONTINUE
      IF(ABS(DNN)-ABS(DSN) .LT. 0) GOTO 212
      IF(ABS(DNN)-ABS(DSN) .EQ. 0) GOTO 212
      IF(ABS(DNN)-ABS(DSN) .GT. 0) GOTO 211
  ...
  216 CONTINUE
  217 CONTINUE
      IF(NBDCND .LT. 0) GOTO 220
      IF(NBDCND .EQ. 0) GOTO 218
      IF(NBDCND .GT. 0) GOTO 220
  218 DO 219 I=1,MP1
         F(I,JPF+1) = F(I,JPS)
  219 CONTINUE
  220 RETURN
...
C     IF (N.LE.2 .OR. I.GT.1) IERROR = 2
  ...
  105 RETURN
     ...
     1                   W,TRI)
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8       Q(IDIMQ,1)
      REAL*8       BA(1)      ,BB(1)      ,BC(1)      ,TWOCOS(1)  ,
     1                B(1)       ,T(1)       ,D(1)       ,W(1)
      EXTERNAL TRI
